...
public class DigesterParameterFactoryTest {
	...
	public void parseRobotWithFriend() throws Exception {
		URL robotWithFriendURL = getClass()
				.getResource("./RobotWithFriend.xml");
		File robotWithFriendFile = new File(new URI(robotWithFriendURL
				.toString()));
		...
		List<Object[]> parmSets = testFactory.createParameters(null,
				robotWithFriendFile);
		...
		assertEquals("The correct number of beans were not created.", 2,
				parmSet.length);

		assertTrue("The first bean is not a Robot.",
				parmSet[0] instanceof Robot);
		assertTrue("The second bean is not a Robot.",
				parmSet[1] instanceof Robot);

		// Verify the first robot
		assertEquals("The first robot does not equal the expected robot.",
				expectedRobot.toString(), parmSet[0].toString());
		assertEquals("The second robot does not equal the expected robot.",
				expectedRobot.toString(), parmSet[1].toString());
	}
